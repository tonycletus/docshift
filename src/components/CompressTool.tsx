...
import type { CompressionLevel } from "@/lib/pdfjs";
...
const LEVELS: {
  ...
  { key: "low", label: "Low", tagline: "Best quality", detail: "Light compression, near-original fidelity.", icon: Feather },
  { key: "medium", label: "Medium", tagline: "Balanced", detail: "Solid quality with a meaningful size cut.", icon: Scale },
  { key: "high", label: "High", tagline: "Smallest file", detail: "Maximum compression — ideal for sharing.", icon: Minimize2 },
];
    ...
    try {
      ...
      const result = await processTool(
        ...
        (pct) => {
          setProgress(pct);
          if (pct >= 10 && pct < 90) {
            // Processor maps page progress to 10..90 range.
            const ratio = (pct - 10) / 80;
            const page = Math.min(totalPages, Math.max(1, Math.round(ratio * totalPages)));
            setProgressLabel(`Processing page ${page} of ${totalPages}…`);
          } else if (pct >= 90) {
            setProgressLabel("Rebuilding compressed PDF…");
          }
        },
      );
        ...
        {status !== "success" && (
          ...
          >
            <div
              {...getRootProps()}
              className={cn(
                "group relative flex min-h-[260px] cursor-pointer flex-col items-center justify-center rounded-[20px] border border-dashed bg-surface/50 px-8 py-12 text-center transition-all duration-200",
                isDragActive
          ...
          >
            ...
            {file.size >= LARGE_FILE_THRESHOLD && (
              <div className="flex items-start gap-2 border-t border-border bg-warning/5 px-4 py-3 text-[12.5px] text-foreground">
                ...
                <span>
                  This is a large PDF ({formatBytes(file.size)}). Processing happens in your browser and may take a
                  moment — please don't close the tab.
                </span>
              </div>
            )}
          </motion.div>
            ...
            {LEVELS.map((l) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[12.5px] leading-snug text-muted-foreground">{l.detail}</span>
                  {active && (
                    <motion.span
                      layoutId="level-dot"
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-[12px] text-muted-foreground">Working locally — your file never leaves your device.</div>
              </div>
        ...
        {status === "success" && resultUrl && (
          ...
          >
            ...
            <div className="mt-4 font-display text-lg font-semibold tracking-tight">Compressed</div>
            ...
            <div className="mx-auto mt-6 grid max-w-md grid-cols-3 gap-px overflow-hidden rounded-2xl border border-border bg-border text-left">
              <div className="bg-background p-4">
                <div className="font-mono text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Original</div>
                <div className="mt-1 font-display text-[15px] font-semibold tracking-tight text-foreground">
                  {formatBytes(originalSize)}
                </div>
              </div>
              <div className="bg-background p-4">
                <div className="font-mono text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Compressed</div>
                <div className="mt-1 font-display text-[15px] font-semibold tracking-tight text-foreground">
                  {formatBytes(resultSize)}
                </div>
              </div>
              <div className="bg-background p-4">
                <div className="font-mono text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground">Saved</div>
                <div
                  className={cn(
        ...
        {status === "error" && (
          ...
          >
            ...
            <div className="flex-1">
              <div className="text-[13.5px] font-medium text-foreground">Couldn't compress that</div>
